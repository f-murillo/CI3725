...
class Asig(ASTNode):
    ...
    def analyze(self, env):
        # LHS (lado izquierdo de la asignacion)
        try:
        ...
        # RHS (lado derecho de la asignacion)
        try:
...
class If(ASTNode):
    ...
    def pretty(self, indent=0):
        ...
        # imprimimos las N-1 lineas Guard
        for depth in range(1, N):
            self._print("Guard", indent+depth)
        # luego imprimimos los Then
...
class FuncInit(ASTNode):
    ...
    def analyze(self, env):
        ...
        # Chequeo de cada par indice:valor
        for idx, val in self.elems:
            # el indice debe ser int
            it = idx.analyze(env)
            if it != "int":
                raise ContextError(
            ...
            # 2el valor tambien debe ser int
            vt = val.analyze(env)
            if vt != "int":
                raise ContextError(
                    f"Expected expression of type int at "
        ...
        # Si todo esta ok, el tipo resultante es el de la funcion base
        self.type = base_t
        return self.type
...
class BinOp(ASTNode):
    ...
    def analyze(self, env):
        ...
        # Comma 
        if self.op == "Comma":
            # dos enteros -> función literal de length=2
            if lt == "int" and rt == "int":
                length = 2

            # int + funcion_literal extiende en 1
            elif lt == "int" and rt.startswith("function with length="):
                prev = int(rt.split("=")[1])
                length = prev + 1

            # funcion_literal + int extiende en 1 tambien
        ...
        # si se llega a este punto, el operador es desconocido
        raise ContextError(
            f"Type error at line {self.lineno} and column {self.column}"
        )
