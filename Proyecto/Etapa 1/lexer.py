...
#--------- Definicion de reglas de tokens (para PLY) ----------------
...
# Tokens de un solo caracter u otros simbolos
t_TkPlus      = r'\+'
t_TkMinus     = r'-'
t_TkMult      = r'\*'
t_TkNot       = r'!'
...
t_TkCBracket  = r'\]'
t_TkTwoPoints = r':'
t_TkApp       = r'\.'  # Este se define despues de TkSoForth para evitar conflictos de solapamiento del simbolo \ por parte de PLY
...
# Funcion que maneja los numeros enteros
def t_TkNum(t):
...
def t_newline(t):
    r'\n+' # Se reconoce uno o mas saltos de linea
    t.lexer.lineno += len(t.value) # Se actualiza el contador de lineas
...
def t_error(t):
    ...
    # Se omite el caracter actual y se continua con el analisis
    t.lexer.skip(1)
...
def main():
    ...
    if not filename.endswith(".imperat"):
        print("Error: La extension del archivo debe ser .imperat")
        sys.exit(1)
